...
TH2F* buildcovmat_th(string label, bool donuc)
{
   int nbins;
   if (label=="ym" || label=="yp" || label=="ch") nbins=10;
   else nbins=5;

   TH2F *hcov = new TH2F(Form("hcov2_%s_%s_ch1",label.c_str(),donuc ? "nuc" : "nonuc"),"hcov2",nbins,0,nbins,nbins,0,nbins);

#define nscale 3
#define nct10 26
#define neps09 41
   double (*xsecp_scale)[2*nscale+1] = new double[10][2*nscale+1];
   double (*xsecm_scale)[2*nscale+1] = new double[10][2*nscale+1];
   double (*A_scale)[2*nscale+1] = new double[nbins][2*nscale+1];
   double (*xsecp_ct10)[2*nct10+1] = new double[10][2*nct10+1];
   double (*xsecm_ct10)[2*nct10+1] = new double[10][2*nct10+1];
   double (*A_ct10)[2*nct10+1] = new double[nbins][2*nct10+1];
   double (*xsecp_eps09)[2*neps09+1] = new double[10][2*neps09+1];
   double (*xsecm_eps09)[2*neps09+1] = new double[10][2*neps09+1];
   double (*A_eps09)[2*neps09+1] = new double[nbins][2*neps09+1];

   ifstream fp_scale("All_Wplus_muon_scales.dta"), fp_ct10("All_Wplus_muon_CT10.dta"), fp_eps09("All_Wplus_muon.dta");
   ifstream fm_scale("All_Wminus_muon_scales.dta"), fm_ct10("All_Wminus_muon_CT10.dta"), fm_eps09("All_Wminus_muon.dta");

   double dummy;

   for (int i=0; i<10; i++)
   {
      fp_scale >> dummy >> dummy >> dummy;
      fm_scale >> dummy >> dummy >> dummy;
      fp_ct10 >> dummy >> dummy >> dummy;
      fm_ct10 >> dummy >> dummy >> dummy;
      fp_eps09 >> dummy >> dummy >> dummy;
      fm_eps09 >> dummy >> dummy >> dummy;

      for (int j=0; j<2*nscale+1; j++)
      {
         fp_scale >> xsecp_scale[i][j];
         fm_scale >> xsecm_scale[i][j];
      }
      for (int j=0; j<2*nct10+1; j++)
      {
         fp_ct10 >> xsecp_ct10[i][j];
         fm_ct10 >> xsecm_ct10[i][j];
      }
      for (int j=0; j<2*neps09+1; j++)
      {
         fp_eps09 >> xsecp_eps09[i][j];
         fm_eps09 >> xsecm_eps09[i][j];
      }
   }
   ...
   for (int i=0; i<nbins; i++)
   {
      if (label=="ym")
      {
         for (int j=0; j<2*nscale+1; j++) A_scale[i][j] = xsecm_scale[i][j]*208*LUMI*1e-6;
         for (int j=0; j<2*nct10+1; j++) A_ct10[i][j] = xsecm_ct10[i][j]*208*LUMI*1e-6;
         for (int j=0; j<2*neps09+1; j++) A_eps09[i][j] = xsecm_eps09[i][j]*208*LUMI*1e-6;
      }
      if (label=="yp")
      {
         for (int j=0; j<2*nscale+1; j++) A_scale[i][j] = xsecp_scale[i][j]*208*LUMI*1e-6;
         for (int j=0; j<2*nct10+1; j++) A_ct10[i][j] = xsecp_ct10[i][j]*208*LUMI*1e-6;
         for (int j=0; j<2*neps09+1; j++) A_eps09[i][j] = xsecp_eps09[i][j]*208*LUMI*1e-6;
      }
      if (label=="ch")
      {
         for (int j=0; j<2*nscale+1; j++) A_scale[i][j] = (xsecp_scale[i][j]-xsecm_scale[i][j])/(xsecp_scale[i][j]+xsecm_scale[i][j]);
         for (int j=0; j<2*nct10+1; j++) A_ct10[i][j] = (xsecp_ct10[i][j]-xsecm_ct10[i][j])/(xsecp_ct10[i][j]+xsecm_ct10[i][j]);
         for (int j=0; j<2*neps09+1; j++) A_eps09[i][j] = (xsecp_eps09[i][j]-xsecm_eps09[i][j])/(xsecp_eps09[i][j]+xsecm_eps09[i][j]);
      }
      if (label=="A1p")
      {
         for (int j=0; j<2*nscale+1; j++) A_scale[i][j] = (xsecp_scale[5+i][j])/(xsecp_scale[4-i][j]);
         for (int j=0; j<2*nct10+1; j++) A_ct10[i][j] = (xsecp_ct10[5+i][j])/(xsecp_ct10[4-i][j]);
         for (int j=0; j<2*neps09+1; j++) A_eps09[i][j] = (xsecp_eps09[5+i][j])/(xsecp_eps09[4-i][j]);
      }
      if (label=="A1m")
      {
         for (int j=0; j<2*nscale+1; j++) A_scale[i][j] = (xsecm_scale[5+i][j])/(xsecm_scale[4-i][j]);
         for (int j=0; j<2*nct10+1; j++) A_ct10[i][j] = (xsecm_ct10[5+i][j])/(xsecm_ct10[4-i][j]);
         for (int j=0; j<2*neps09+1; j++) A_eps09[i][j] = (xsecm_eps09[5+i][j])/(xsecm_eps09[4-i][j]);
      }
      if (label=="A3")
      {
         for (int j=0; j<2*nscale+1; j++) A_scale[i][j] = (xsecp_scale[5+i][j]+xsecm_scale[5+i][j])/(xsecp_scale[4-i][j]+xsecm_scale[4-i][j]);
         for (int j=0; j<2*nct10+1; j++) A_ct10[i][j] = (xsecp_ct10[5+i][j]+xsecm_ct10[5+i][j])/(xsecp_ct10[4-i][j]+xsecm_ct10[4-i][j]);
         for (int j=0; j<2*neps09+1; j++) A_eps09[i][j] = (xsecp_eps09[5+i][j]+xsecm_eps09[5+i][j])/(xsecp_eps09[4-i][j]+xsecm_eps09[4-i][j]);
      }
   }

   for (int i=0; i<nbins; i++) for (int j=0; j<nbins; j++)
   {
      double nnsi = donuc ? 1. : A_ct10[i][0]/A_eps09[i][0];// "no nuc" scale
      double nnsj = donuc ? 1. : A_ct10[j][0]/A_eps09[j][0];// "no nuc" scale
      double cov_ij=0;
      for (int k=0; k<nscale; k++) 
         cov_ij += nnsi*nnsj*0.5*((A_scale[i][1+2*k]-A_scale[i][0])*(A_scale[j][1+2*k]-A_scale[j][0]) + (A_scale[i][2+2*k]-A_scale[i][0])*(A_scale[j][2+2*k]-A_scale[j][0]));
      if (!donuc) 
         for (int k=0; k<nct10; k++) 
            cov_ij += 0.5*(A_ct10[i][1+2*k]-A_ct10[i][0])*(A_ct10[j][1+2*k]-A_ct10[j][0]) + 0.5*(A_ct10[i][2+2*k]-A_ct10[i][0])*(A_ct10[j][2+2*k]-A_ct10[j][0]);
      else 
         for (int k=0; k<neps09; k++) 
            cov_ij += 0.5*(A_eps09[i][1+2*k]-A_eps09[i][0])*(A_eps09[j][1+2*k]-A_eps09[j][0]) + 0.5*(A_eps09[i][2+2*k]-A_eps09[i][0])*(A_eps09[j][2+2*k]-A_eps09[j][0]);

      if (nbins==5) hcov->SetBinContent(i+1,j+1,cov_ij);
      else hcov->SetBinContent(10-i,10-j,cov_ij);

      if (i==j) cout << sqrt(cov_ij) << endl;
   }

   return hcov;
}

void chi2_recomputeth(const char* file_graphs, const char* file_matrices_th, int nchan)
{
   TFile *fgr = new TFile(file_graphs);
   TFile *fmat_th = new TFile(file_matrices_th);
   ...
   for (unsigned int i=0; i<names.size(); i++)
   {
      ...
      TH2F *hcov_nuc_th = buildcovmat_th(label2,true); // (TH2F*) fmat_th->Get(Form("matrices/hcov2_%s_nuc_ch%i",label2.c_str(),nchan));
      TH2F *hcov_nonuc_th = buildcovmat_th(label2,false); // (TH2F*) fmat_th->Get(Form("matrices/hcov2_%s_nonuc_ch%i",label2.c_str(),nchan));
      TH2F *hcov_exp = (TH2F*) build_covmat(grstat,gr,(label2=="yp"||label2=="ym")); // We want the luminosity uncertainty in the covariance matrix only for yields
      TH2F *hcov_nuc_all = new TH2F(*hcov_nuc_th);
      hcov_nuc_all->Add(hcov_exp);
      TH2F *hcov_nonuc_all = new TH2F(*hcov_nonuc_th);
