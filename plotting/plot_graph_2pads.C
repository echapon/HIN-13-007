...
void plot_graph_1file(const char* fname="graph.root", const char *gbasename="gyields", int channel_number=0, bool reverteta=false, double scale=1., bool theory_only=false)
{
   ...
   if (!theory_only) gexp_syst1->Draw("||");
   ...
   if (!theory_only) gexp_stat1->Draw("PZ");
   ...
   if (twochan)
   {
      ...
      if (!theory_only) gexp_syst2->Draw("||");
      ...
      if (!theory_only) gexp_stat2->Draw("PZ");
   }
   ...
   if (doAlice)
   {
      ...
      if (!theory_only) galice_syst->Draw("||");
      ...
      if (!theory_only) galice_stat->Draw("PZ");
   }
   ...
   if (!theory_only) tleg2->AddEntry(gexp_syst1,legend1.c_str(),"p");
   if (twochan) 
   {
      ...
      if (!theory_only) tleg2->AddEntry(gexp_syst2,legend2.c_str(),"p");
   }
   else
   {
      // if (string(gbasename)=="gyieldsp"||string(gbasename)=="gyieldsm")
   ...
   CMS_lumi( (TPad*) gPad, iPeriod, iPos );
...
void plot_graph_1file_all(const char* filename, int chan, bool reverse_eta=true, double scale=1./34.62, bool theory_only=false)
{
   plot_graph_1file(filename,"gA3",chan,false,1.,theory_only);
   plot_graph_1file(filename,"gch",chan,reverse_eta,1.,theory_only);
   plot_graph_1file(filename,"gA1p",chan,false,1.,theory_only);
   plot_graph_1file(filename,"gA1m",chan,false,1.,theory_only);
   plot_graph_1file(filename,"gA4",chan,false,1.,theory_only);
   plot_graph_1file(filename,"gyieldsp",chan,reverse_eta,scale,theory_only);
   plot_graph_1file(filename,"gyieldsm",chan,reverse_eta,scale,theory_only);
}
